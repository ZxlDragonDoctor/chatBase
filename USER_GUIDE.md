...
- 发送 `/help` 查看所有命令
- 发送 `/new` 或 `/重置` 重置当前会话
- 发送 `/history 10` 查看最近 10 条对话
- 发送 `/feedback 建议改进回复速度` 提交反馈
...
> **说明**：系统已关闭自助注册。新账号由管理员在「控制台 → 用户管理」中创建并分发。

#### 7.1.1 管理员创建账号

1. 使用管理员账号登录
2. 打开 `/console/admin/users`（用户管理）
3. 点击「创建用户」
4. 填写用户名、初始密码（至少 6 位）、昵称/邮箱（可选）
5. 将用户名与初始密码告知使用者
