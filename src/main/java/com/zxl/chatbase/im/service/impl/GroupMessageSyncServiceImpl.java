...
public class GroupMessageSyncServiceImpl extends ServiceImpl<GroupMessageMapper, GroupMessage> implements GroupMessageSyncService {
    ...
    @Override
    public void savePrivateMessage(String platform, String messageId, String userId,
                                    String rawMessage, String messageType, long time,
        ...
        try {
            if (messageId != null && !messageId.isEmpty()) {
                ...
                GroupMessage existing = groupMessageMapper.selectOne(wrapper);
                if (existing != null) {
                    log.info("私聊消息已存在，跳过保存: platform={}, messageId={}", platform, messageId);
                    return;
                }
            }

            if (!StringUtils.hasText(conversationId)) {
                conversationId = "single:" + platform + ":" + userId;
            }
            ...
            gm.setCreateTime(LocalDateTime.now());
            groupMessageMapper.insert(gm);
            log.info("私聊消息写库成功: platform={}, conversationId={}, userId={}, type={}", platform, conversationId, userId, messageType);
        } catch (Exception e) {
            log.error("保存私聊消息失败: platform={}, messageId={}", platform, messageId, e);
        }
    }
