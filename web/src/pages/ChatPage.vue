    ...
    <section class="anime-card anime-chat-card">
      ...
      <div class="anime-chat-layout">
        ...
        <div class="anime-chat-main">
          <div class="anime-chat-messages">
            ...
            <div v-for="(m, idx) in messages" :key="idx" class="anime-chat-message anime-fade-in" :class="m.role">
              ...
              <div class="anime-chat-bubble">
                ...
                <div v-if="m.sources?.length" style="margin-top: 12px; padding-top: 12px; border-top: 1px solid var(--anime-border-light);">
                  <div style="margin-bottom: 8px;">
                    <span class="anime-badge blue">引用来源</span>
                  </div>
                  <div v-for="(s, i) in m.sources" :key="i" style="padding: 12px; background: rgba(168, 216, 234, 0.08); box-shadow: var(--anime-shadow-sm); border-radius: var(--anime-radius-lg); margin-bottom: 8px;">
          ...
          <div v-if="pendingFiles.length" style="padding: 14px 20px; background: var(--anime-pink-bg); border-top: 1px solid var(--anime-border-light); display: flex; align-items: center; gap: 12px; flex-wrap: wrap;">
          ...
          <div class="anime-chat-composer">
            ...
            <div v-if="showAttachMenu" style="position: absolute; bottom: 100%; left: 0; background: var(--anime-bg-card); border-radius: var(--anime-radius-lg); padding: 10px; display: flex; flex-direction: column; gap: 8px; z-index: 10; box-shadow: var(--anime-shadow-pink);">
...
.anime-chat-sidebar { width: 260px; background: var(--anime-bg-card); overflow-y: auto; position: relative; }
.anime-chat-sidebar::after { content: ''; position: absolute; right: 0; top: 0; bottom: 0; width: 1px; background: linear-gradient(180deg, transparent, var(--anime-border), transparent); }
...
.anime-session-item:hover { box-shadow: var(--anime-shadow-pink); }
.anime-session-item.active { box-shadow: var(--anime-shadow-pink); background: var(--anime-pink-bg); }
...
.thinking-section {
  ...
  box-shadow: var(--anime-shadow-sm);
  border-radius: var(--anime-radius-lg);
  background: var(--anime-purple-bg);
}
...
.message-content blockquote {
  border-left: 3px solid var(--anime-pink);
  padding-left: 12px;
  margin: 12px 0;
  color: var(--anime-text-secondary);
